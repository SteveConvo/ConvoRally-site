  ...
  return(
    <div style={{background:bg,color:ink,minHeight:"100vh",fontFamily:"'DM Sans', sans-serif",overflowX:"hidden"}}>
      ...
      <section style={{minHeight:"92vh",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",textAlign:"center",padding:"130px 24px 80px"}}>
        ...
        <FadeIn delay={0.08}><h1 style={{fontFamily:"'Instrument Serif', Georgia, serif",fontSize:"clamp(36px, 5.5vw, 66px)",fontWeight:400,lineHeight:1.08,maxWidth:720,marginBottom:24,letterSpacing:"-0.02em"}}>Your evidence is only as strong <span style={{fontStyle:"italic",color:red}}>as your documentation.</span></h1></FadeIn>
        <FadeIn delay={0.18}><p style={{fontSize:19,color:mid,maxWidth:540,lineHeight:1.7,marginBottom:36}}>Forensic investigations involve multiple experts, attorneys, insurers, and property owners — all creating records that need to be time-stamped, attributed, and organized. ConvoRally helps keep the documentation clear from day one.</p></FadeIn>
      ...
      <section style={{background:navy,color:"#fff",padding:"70px 24px"}}>
        <div style={{maxWidth:880,margin:"0 auto"}}>
          <FadeIn><div style={{textAlign:"center",marginBottom:48}}><h2 style={{fontFamily:"'Instrument Serif', Georgia, serif",fontSize:"clamp(26px, 3.5vw, 38px)",fontWeight:400,lineHeight:1.25}}>In forensics, the record <span style={{fontStyle:"italic",color:"#FCA5A5"}}>matters as much as the finding.</span></h2></div></FadeIn>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))",gap:24}}>
            {[{title:"Long timelines",desc:"Investigations can stretch over months or years. Every communication and finding must be traceable long after the fact.",color:"#FCA5A5"},{title:"Many contributors",desc:"Multiple parties contribute notes, photos, reports, and decisions. Keeping it organized matters.",color:"#7DD3FC"},{title:"Record quality",desc:"The quality of the documentation can shape how credible the case looks later.",color:"#FCD34D"}].map((item,i)=>(
      ...
      <section style={{padding:"90px 24px",maxWidth:800,margin:"0 auto"}}>
        <FadeIn><div style={{textAlign:"center",marginBottom:56}}><p style={{fontSize:13,fontWeight:600,color:teal,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:14}}>Built for Structured Documentation</p><h2 style={{fontFamily:"'Instrument Serif', Georgia, serif",fontSize:"clamp(26px, 3.5vw, 40px)",fontWeight:400,lineHeight:1.2}}>From initial observation to final report — <span style={{fontStyle:"italic",color:teal}}>a structured record.</span></h2></div></FadeIn>
        {[
          {icon:"\u{1F4F8}",title:"Time-stamped site documentation",desc:"Inspections, observations, and site conditions are captured with timestamps and attribution.",color:teal},
          {icon:"\u{1F517}",title:"Clear sequence of findings",desc:"Every contribution is logged in order, making it easier to see who identified what and when.",color:red},
          {icon:"\u{1F465}",title:"Multi-party coordination",desc:"Engineers, labs, attorneys, insurers, and property owners can each have scoped access to the same matter.",color:gold},
          {icon:"\u{1F4CB}",title:"Report-ready organization",desc:"Findings, results, and conclusions build over time in a structured, searchable format.",color:navy},
          {icon:"\u{1F512}",title:"Organized under scrutiny",desc:"When questions arise later, the record is easier to follow because it was built in sequence.",color:"#16a34a"},
        ].map((item,i)=>(
      ...
      <section style={{padding:"80px 24px",background:"#ECEAE5"}}>
        <div style={{maxWidth:680,margin:"0 auto"}}>
          <FadeIn><div style={{textAlign:"center",marginBottom:44}}><h2 style={{fontFamily:"'Instrument Serif', Georgia, serif",fontSize:"clamp(26px, 3.5vw, 38px)",fontWeight:400,lineHeight:1.25}}>The difference between winning and losing a case <span style={{fontStyle:"italic",color:red}}>is often the documentation.</span></h2></div></FadeIn>
          {[
            {quote:"We had clear evidence of construction defects, but the timeline of our communications was scattered across three email accounts. Opposing counsel picked it apart.",role:"Forensic engineer, 22 years"},
            {quote:"When multiple experts contribute to an investigation, version control becomes a nightmare. Who saw what, when, and what was the conclusion at that point?",role:"Construction defect attorney"},
            {quote:"Our insurer disputed the claim because we couldn't prove when the damage was first identified.",role:"Property owner, commercial building"},
          ].map((item,i)=>(
            <FadeIn key={i} delay={i*0.1}><div style={{position:"relative",paddingLeft:28,marginBottom:28}}><div style={{position:"absolute",left:0,top:0,width:4,height:"100%",borderRadius:2,background:i===0?red:i===1?teal:gold}} /><p style={{fontFamily:"'Instrument Serif', Georgia, serif",fontSize:19,fontStyle:"italic",color:ink,lineHeight:1.55,marginBottom:8}}>{'"'}{item.quote}{'"'}</p><p style={{fontSize:13,color:soft}}>— {item.role}</p></div></FadeIn>
          ))}
        </div>
      </section>
      ...
      <section style={{padding:"80px 24px",maxWidth:920,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(190px, 1fr))",gap:16}}>
          {[
            {role:"Forensic Engineers",desc:"Document findings, coordinate contributors, and keep a clear trail of observations.",color:red},
            {role:"Construction Attorneys",desc:"Access investigation records with better attribution and scoped visibility.",color:teal},
            {role:"Insurance Adjusters",desc:"See when damage was reported, inspected, and assessed.",color:gold},
            {role:"Property Owners",desc:"Track what experts found, when they found it, and what happens next.",color:navy},
            {role:"Testing Laboratories",desc:"Submit results into the record with timestamps and attribution.",color:mid},
          ].map((item,i)=>(
      ...
      <section id="cta" style={{background:navy,color:"#fff",padding:"90px 24px",textAlign:"center"}}>
        ...
        <FadeIn><h2 style={{fontFamily:"'Instrument Serif', Georgia, serif",fontSize:"clamp(26px, 4vw, 42px)",fontWeight:400,lineHeight:1.2,marginBottom:8}}>Build the record before you need it.</h2></FadeIn>
        <FadeIn><p style={{fontSize:15,color:"rgba(255,255,255,0.5)",marginBottom:6}}>Join the Founding 100</p><p style={{fontSize:17,color:"rgba(255,255,255,0.7)",maxWidth:440,margin:"0 auto 28px",lineHeight:1.6}}>Be one of the first 100 forensic professionals shaping ConvoRally for investigations and expert analysis.</p></FadeIn>
